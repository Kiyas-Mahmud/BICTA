...
- **Public write endpoints (only three):** `POST /api/registrations`, `POST /api/newsletter`, `POST /api/contact` — all order: rate limit → honeypot/time-trap → Zod → dedupe/window checks → insert. Any new public write endpoint requires explicit security review.
